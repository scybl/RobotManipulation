/* Shape-sorting perception pipeline implementation. */
...
ShapeSortingSolution::ShapeSortingSolution(const rclcpp::Node::SharedPtr &node)
: node_(node),
  ...
  g_cloud_ptr(new PointC)
{
  t1_service_ = node_->create_service<cw2_world_spawner::srv::Task1Service>(
    "/task1_start",
    std::bind(&ShapeSortingSolution::t1_callback, this, std::placeholders::_1, std::placeholders::_2));
  t2_service_ = node_->create_service<cw2_world_spawner::srv::Task2Service>(
    "/task2_start",
    std::bind(&ShapeSortingSolution::t2_callback, this, std::placeholders::_1, std::placeholders::_2));
  t3_service_ = node_->create_service<cw2_world_spawner::srv::Task3Service>(
    "/task3_start",
    std::bind(&ShapeSortingSolution::t3_callback, this, std::placeholders::_1, std::placeholders::_2));
  ...
  color_cloud_sub_ = node_->create_subscription<sensor_msgs::msg::PointCloud2>(
    pointcloud_topic_,
    pointcloud_qos,
    std::bind(&ShapeSortingSolution::cloud_callback, this, std::placeholders::_1),
    pointcloud_sub_options);
  joint_state_sub_ = node_->create_subscription<sensor_msgs::msg::JointState>(
    "/joint_states",
    rclcpp::SensorDataQoS(),
    std::bind(&ShapeSortingSolution::joint_state_callback, this, std::placeholders::_1));
  ...
  RCLCPP_INFO(
    node_->get_logger(),
    "shape_sorting_solution initialised with pointcloud topic '%s' (%s QoS)",
    pointcloud_topic_.c_str(),
    pointcloud_qos_reliable_ ? "reliable" : "sensor-data");
}

void ShapeSortingSolution::cloud_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
...
void ShapeSortingSolution::joint_state_callback(const sensor_msgs::msg::JointState::ConstSharedPtr msg)
{
...
bool ShapeSortingSolution::move_arm_to_named_target(const std::string &target_name)
{
...
bool ShapeSortingSolution::move_arm_to_pose(const geometry_msgs::msg::Pose &pose, const std::string &frame_id)
{
...
bool ShapeSortingSolution::execute_cartesian_path(
...
bool ShapeSortingSolution::set_gripper_width(const double width)
{
  hand_group_->startStateMonitor();
...
double ShapeSortingSolution::get_gripper_width() const
{
  std::lock_guard<std::mutex> lock(joint_state_mutex_);
...
void ShapeSortingSolution::attach_grasped_object_collision(const std::string &shape_type)
{
...
void ShapeSortingSolution::detach_grasped_object_collision()
{
  moveit_msgs::msg::AttachedCollisionObject attached_object;
  attached_object.link_name = "panda_link8";
  attached_object.object.id = kAttachedObjectId;
...
bool ShapeSortingSolution::rescan_task1_object_point(
...
bool ShapeSortingSolution::estimate_task1_object_yaw(
...
bool ShapeSortingSolution::extract_task2_object_cloud(
  const geometry_msgs::msg::PointStamped &object_point,
  PointC &object_cloud)
{
  PointCPtr cloud;
...
bool ShapeSortingSolution::build_task2_shape_signature(
...
double ShapeSortingSolution::compare_task2_shape_signatures(
...
bool ShapeSortingSolution::build_task2_scan_pose(
  const geometry_msgs::msg::PointStamped &object_point,
...
std::string ShapeSortingSolution::classify_task2_shape_pairwise(
...
bool ShapeSortingSolution::observe_task2_shape(
  const geometry_msgs::msg::PointStamped &object_point,
...
geometry_msgs::msg::Pose ShapeSortingSolution::make_top_down_pose(
  const double x,
  const double y,
  const double z,
...
bool ShapeSortingSolution::execute_pick_and_place_sequence(
  const geometry_msgs::msg::Point &object_point,
...
void ShapeSortingSolution::t1_callback(
...
void ShapeSortingSolution::t2_callback(
...
void ShapeSortingSolution::t3_callback(
...
bool ShapeSortingSolution::t3_collect_scene_cloud(PointCPtr &merged_cloud, const std::string &target_frame)
{
...
void ShapeSortingSolution::t3_cluster_cloud(const PointCPtr &cloud, std::vector<PointCPtr> &clusters)
{
  clusters.clear();
...
std::string ShapeSortingSolution::t3_classify_cluster(
...
bool ShapeSortingSolution::t3_find_basket_pos(const PointCPtr &basket_cloud, geometry_msgs::msg::Point &basket_pos)
{
...
void ShapeSortingSolution::t3_register_obstacle(const PointCPtr &cluster, const std::string &id)
{
...
void ShapeSortingSolution::t3_clear_obstacles(const std::vector<std::string> &ids)
{
  if (!ids.empty()) {
...
bool ShapeSortingSolution::t3_pick_and_place(
